...
text-to-3D generative model (Tripo3D) without exposing API credentials to
...
much better fit for an interactive AR loop. We use Tripo3D's hosted
text-to-3D API, a commercial system in this same feed-forward family,
because it exposes a stable REST API and returns GLB meshes directly
usable by a WebGL/WebXR renderer, letting us focus engineering effort on
the AR integration rather than on training or hosting a generative model
ourselves. (An initial implementation targeted Meshy AI instead; we
switched after finding Meshy's REST API requires prepaid credits with no
free tier, whereas Tripo3D grants new API accounts 300 free credits. The
proxy server's provider-specific code is isolated behind the same
\texttt{/api/generate}, \texttt{/api/status}, \texttt{/api/model} contract
either way, so the swap did not touch the client at all.)
...
\begin{itemize}
  ...
  (b) creates and polls Tripo3D text-to-3D tasks on the client's behalf,
  and (c) fetches and caches the resulting GLB so the Tripo3D API key is
  never sent to the browser and the client never talks to a third-party
  origin directly.
  \item \textbf{Generative model}: Tripo3D's hosted text-to-3D endpoint,
  called with texturing disabled (untextured geometry only) to minimize
  latency in the interactive loop.
\end{itemize}

\subsection{Generation pipeline}
On submitting a prompt, the client \texttt{POST}s to \texttt{/api/generate};
the proxy creates a Tripo3D generation task and returns a task id. The
client polls \texttt{/api/status/\{id\}} every two seconds, updating a
visible progress bar from Tripo3D's reported completion percentage. On success, the
...
% TODO(fill in after live testing with a real Tripo3D API key + real webcam):
...
%    rapid prompts, behaviour when Tripo3D is slow/unavailable
...
Tripo3D's texture option (disabled here, to keep the interactive loop
...
no license needed); and Tripo3D's~\cite{tripo3d} hosted text-to-3D REST
API (commercial, used via its documented API under our own account -- no
model weights or proprietary code from Tripo3D are redistributed). All AR interaction logic,
